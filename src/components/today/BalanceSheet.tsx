...
export function BalanceSheet({
  ...
  // Reference material behind the (i) in the header; per-open, not
  // persisted: unlike the daily plan note there's nothing to re-read daily.
    ...
    >
      ...
      >
          ...
          <div className="mb-1 flex items-start justify-between gap-3">
            <div className="flex min-w-0 items-center">
              ...
              <button
                ...
                aria-label="How balancing works"
                aria-expanded={showInfo}
                aria-controls="balance-info"
                className="press flex h-9 w-9 shrink-0 items-center justify-center rounded-full text-(--muted) hover:bg-(--track) hover:text-(--ink)"
              >
        ...
        <div ref={scrollRef} className="min-h-0 flex-1 overflow-y-auto px-5 pb-8 pt-2">
          {/* Same 0fr -> 1fr disclosure mechanic as DaySummaryNote, same
              tint box: one vocabulary for "here's why", app-wide. */}
          <div
            id="balance-info"
            className="grid transition-[grid-template-rows] duration-300 ease-[cubic-bezier(0.16,1,0.3,1)] motion-reduce:transition-none"
            style={{ gridTemplateRows: showInfo ? "1fr" : "0fr" }}
          >
            <div
              className="overflow-hidden transition-opacity duration-200 motion-reduce:transition-none"
              style={{ opacity: showInfo ? 1 : 0 }}
            >
              <div className="mb-3 rounded-3xl bg-(--tint) px-4 py-3">
                <p className="text-sm leading-6 text-(--tint-ink)">
                  A day that goes over gets spread across the rest of its week, so no single day
                  has to make it up. Each day gives up at most 10% of its target, never below your
                  safety floor, and whatever doesn&apos;t fit is forgiven, not carried.
                </p>
                <p className="mt-2 text-sm leading-6 text-(--tint-ink)">
                  Quick estimates assume a night of mostly carbs and fat: about 10% protein, 55%
                  carbs, 35% fat, with drinks counted as carbs. Protein targets never move; trims
                  come out of carbs and fat.
                </p>
              </div>
            </div>
          </div>
