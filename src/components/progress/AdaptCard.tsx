...
export function AdaptCard({ state, busy, onResolve, accepted }: Props) {
  ...
  if (accepted) {
    return (
      <Card title="Adaptive target">
        <p className="text-sm leading-6 text-[#2c3a2e]">
          Done. Your daily target is now <span className="font-semibold">{accepted.newKcal} kcal</span>.
        </p>
        <p className="mt-2 text-sm leading-6 text-[#5d6b5f]">{accepted.explanation}</p>
  ...
  if (proposal) {
    ...
    return (
      <Card title="Adaptive target">
        <p className="text-sm font-medium leading-6 text-[#2c3a2e]">
          Suggestion: {direction} your daily target by {Math.abs(proposal.correctionDelta)} kcal to{" "}
          {proposal.previewKcal} kcal.
        </p>
        <p className="mt-2 text-sm leading-6 text-[#5d6b5f]">{proposal.rationale}</p>
        ...
        <button
          ...
          className="mt-2 text-xs text-[#7a9a4e] underline-offset-2 hover:underline"
        >
          {showMath ? "Hide the details" : "How we worked this out"}
        </button>
        {showMath && (
          <ul className="mt-2 space-y-1 text-xs text-[#5d6b5f]">
        ...
        <div className="mt-4 flex gap-2">
          <button
            ...
            disabled={busy}
            className="press flex-1 rounded-2xl bg-[#2c3a2e] px-4 py-3 text-sm font-medium text-white disabled:opacity-60"
          >
          ...
          <button
            ...
            disabled={busy}
            className="press rounded-2xl border border-[#dce3d7] bg-white px-4 py-3 text-sm text-[#2c3a2e] disabled:opacity-50"
          >
            Not now
          </button>
        </div>
  ...
  return (
    <Card title="Adaptive target">
      ...
      <ul className="mt-3 space-y-1 text-sm text-[#5d6b5f]">
        <li>
...
function Card({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <section className="rounded-3xl bg-white p-5 shadow-sm">
      <h2 className="mb-2 text-lg font-semibold text-[#2c3a2e]">{title}</h2>
      {children}
    </section>
  );
}
